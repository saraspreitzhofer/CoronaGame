...
class Game: # test
    ...
    def draw(self): # game loop - draw
        ...
        self.WIN.blit(self.runner.image, (self.runner.rect.x, self.runner.rect.y))  # draw surface (pictures, text, ...) on the screen
...
class Runner(pygame.sprite.Sprite): # Runner is a pygame Sprite object
    def __init__(self): # runs whenever a new object of this type is made
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.transform.scale(self.pygame.image.load(os.path.join('assets', "runner.png")), (RUNNER_WIDTH, RUNNER_HEIGHT))  # resize image
        self.rect = pygame.Rect(10, 370, RUNNER_WIDTH, RUNNER_HEIGHT)


    def jump(self):
        self.runner.rect.y -= self.VELOCITY_JUMP
